...
contract tips {
    ...
    function addWitress(address payable walletAddress,string memory name) public{
        require(msg.sender == owner, "Only the owner can call this function");   ///เพิ่มคนได้เฉพาะเจ้าของ
    ...
    function distrubiteTips() public{ 
        require(address(this).balance> 0, "Insufficient balance in the contract");   ///กดไม่ได้ถ้าไม่มีเงิน
        if(waitress.length>=1){
